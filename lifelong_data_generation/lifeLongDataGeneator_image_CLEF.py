...
parser.add_argument('--data_dir', type = str, default = '/media/ta/e9cf3417-0c3e-4e6a-b63c-4401fabeabc8/ta/image_CLEF_split', help = 'Data directory')
parser.add_argument('--task_order', type = int, default = ['b', 'c', 'i', 'p'], help = 'Task order') # ['i', 'p', 'c', 'b']
parser.add_argument('--output_csv', type = str, default = 'image_CLEF_01.csv', help = 'Output csv')
...
parser.add_argument('--output_dir', type = str, default = 'image_CLEF_01/', help = 'Output directory')

args = parser.parse_args()

def main(args):
    task = args.task_order[0]
    classes = os.listdir(os.path.join(args.data_dir, task, f'{args.source}'))
    classes = np.array([str(i) for i in range(len(classes))])
    
    ll_imgs = []
    ll_labels = []
    for task in args.task_order:
        imgs = glob.glob(os.path.join(args.data_dir, task, f'{args.source}', '*/*.*'))
        imgs = [os.path.join(task, '/'.join(img.split('/')[-3:])) for img in imgs]
