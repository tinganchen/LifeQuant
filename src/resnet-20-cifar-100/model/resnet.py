...
class PreActResNet(nn.Module):
  ...
  def forward(self, x):
    ...
    self.feature = self.out.view(self.out.size(0), -1)
    out = self.logit(self.feature)
    return out


def resnet20_quant(bitW, abitW, stage, num_classes=args.num_classes):
  return PreActResNet(PreActBlock_conv_Q, [3, 3, 3], bitW, abitW, stage, num_classes=num_classes)

def resnet56_quant(bitW, abitW, stage, num_classes=args.num_classes):
